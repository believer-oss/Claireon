...
- **`$INPUT_PROPOSAL_FILE`**: Absolute path to the proposal document to break down (e.g., `Docs/SPAWNING.md`)
...
Assume the role of a **senior engineer** working on an Unreal Engine project. Approach the breakdown with:
- Deep familiarity with UE5 module architecture and patterns
...
Create a branch for the work following your project's branching conventions. The branch should be named after the proposal scope (e.g., `feat/spawning-system`).
...
5. **Test type and thoroughness scale with risk**:
   ...
   - **Formatting** -> Run your project's format checker on changed files

6. **Each test stage ends with a commit** following the conventional commit format:
   ```
   <type>(<scope>) [ci]: <description>
   ```
...
## Branch
The branch name will be recorded here after setup.
...
- **[ci]**: Include when source files (.cpp/.h/.cs) changed
...
009-implement-integration.md        — Wire into existing systems
