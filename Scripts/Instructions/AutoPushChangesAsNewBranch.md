...
If there are staged changes to commit, analyze them to create a commit message following the conventional commit format:
```
...
**[ci] flag**:
- Include `[ci]` to trigger CI/CD pipeline when source files changed (cpp/h/cs)
- Omit `[ci]` if only non-source files changed (documentation, config, scripts, etc.)
...
✓ Changes moved to new branch and rebased on main
  ...
  New branch: llm/arcmage404/worktree-3/auth-fixes
  Commits moved: 3
  Latest commit: abc1234
  Message: fix(auth): token validation logic corrected [ci]
  Remote: https://github.com/user/repo/tree/llm/arcmage404/worktree-3/auth-fixes
```
...
New branch: llm/arcmage404/worktree-3/api-routes
...
1. Resolve conflicts and continue:
   git add <resolved-files>
   git rebase --continue
   git push -u origin llm/arcmage404/worktree-3/api-routes
