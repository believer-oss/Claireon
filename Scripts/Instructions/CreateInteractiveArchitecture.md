...
Generate an interactive, self-contained HTML architecture diagram for any project system. The output is a single `.html` file with an SVG diagram, clickable nodes with a detail sidebar, layer filter buttons, connection highlighting, and animated data flow.
...
- Optionally MCP editor tools for blueprint inspection: `search_assets`, `get_blueprint_properties`, `get_blueprint_graph`
...
> "Which system would you like to diagram? (e.g., Components, Subsystems, UI, Networking)"
...
1. `get_blueprint_properties` — Properties, functions, event dispatchers
2. `get_blueprint_graph` with `format="json"` and `detail_level="full"` — Graph nodes and connections
...
- Search for backend SDK usage: service SDK references, server RPC patterns
...
| **cppClass** | C++ class name (e.g., `UMyMenuWidget`) |
| **parentClass** | Parent class (e.g., `UMyMenu`) |
| **assetPath** | Unreal content path if applicable |
| **layer** | One of: `widgets`, `viewmodels`, `gamelogic`, `data`, `subsystems`, `services` |
| **tags** | Array from: `cpp`, `bp`, `vm`, `struct`, `replicated`, `delegate`, `subsystem`, `service`, `rpc` |
...
| Backend / Services | `services` | `#f85149` (red) | `#221722` | Backend service calls, service SDK, server RPCs |
...
<title>{{SYSTEM_NAME}} — Interactive Architecture Diagram</title>
<style>
...
| Creates / wraps VM | `#3fb950` green | `arrow-green` | Display -> VMSlot |
...
- `RPCs` — for controller RPC endpoints
...
An example system diagram demonstrates this style in practice:
