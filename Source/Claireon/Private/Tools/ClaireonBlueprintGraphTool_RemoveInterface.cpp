...
FString ClaireonBlueprintGraphTool_RemoveInterface::GetOperation() const { return TEXT("remove_interface"); }

FString ClaireonBlueprintGraphTool_RemoveInterface::GetDescription() const
{
    return TEXT("Remove an interface from the Blueprint's ImplementedInterfaces in the open editing session and compile. Requires open session_id from bp_open (or pass asset_path to auto-open). Transactional. Common pitfall: any function-override graphs implementing the interface are deleted with it. Accepts either session_id or asset_path; auto-opens a session when asset_path is supplied.");
}
...
FToolResult ClaireonBlueprintGraphTool_RemoveInterface::Execute(const TSharedPtr<FJsonObject>& Arguments)
{
    TSharedPtr<FJsonObject> Params;
    FString SessionId;
    FBlueprintEditToolData* Data = nullptr;
    FToolResult Error;
    if (!BeginSessionOp(Arguments, TEXT("remove_interface"), Params, SessionId, Data, Error))
    {
        return Error;
    }
	...
	{
		return MakeErrorResult(TEXT("Missing required field 'interface_class' for remove_interface"));
	}
	...
	ClaireonBPInterfaceAuthor::FInterfaceOpResult OpResult =
		ClaireonBPInterfaceAuthor::Remove(Blueprint, InterfaceClassName);
	...
	Data->Cursor.LastOperationStatus = FString::Printf(
		TEXT("Removed %s; compiled (%d interfaces total)"),
