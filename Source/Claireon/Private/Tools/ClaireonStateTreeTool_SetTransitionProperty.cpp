...
{
    return TEXT("Set a struct field on an FStateTreeTransition (e.g. bConsumeEventOnSelect, bDelayTransition, DelayDuration, Priority, bTransitionEnabled, RequiredEvent.Tag). Refuses Target/State/StateLink (those belong to statetree_modify_transition). Requires open session_id from statetree_open. Transactional.");
}
