...
FString ClaireonTool_LiveCodingReload::GetDescription() const
{
	...
	// so callers know to escalate to a full rebuild+relaunch in that case.
	return TEXT("Trigger a Live Coding reload for .cpp changes. Non-session and immediate: the reload is deferred "
		"until the current script finishes. Refuses when headers changed -- call claireon.proxy with "
		"command='launch_editor' for the full rebuild+relaunch. KNOWN ENGINE BUG: live-coding through a "
		"UChildActorComponent template can crash on an IsInAsyncLoadingThread() assertion; rebuild instead.");
}
