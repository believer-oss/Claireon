...
{
    return TEXT("Add an interface to the Blueprint's ImplementedInterfaces and compile -- an alias of bp_add_interface "
                "matching the editor's 'Implement Interface' UI label. Prefer bp_add_interface as the canonical name. "
                "Session-mode tool: transactional; pass session_id from bp_open, or asset_path to auto-open one.");
}
