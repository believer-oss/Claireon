...
FString ClaireonBlueprintGraphTool_RenameComponent::GetOperation() const { return TEXT("rename_component"); }

FString ClaireonBlueprintGraphTool_RenameComponent::GetDescription() const
{
    return TEXT("Rename a component variable in the Blueprint's Simple Construction Script in the open editing session. Requires open session_id from bp_open (or pass asset_path to auto-open). Transactional. The new name must be unique within the SCS; references in graph nodes are auto-fixed when possible. Accepts either session_id or asset_path; auto-opens a session when asset_path is supplied.");
}
