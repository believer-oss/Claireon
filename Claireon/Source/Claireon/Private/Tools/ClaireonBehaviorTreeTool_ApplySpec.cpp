...
{
	return TEXT("Apply a declarative JSON specification to a Behavior Tree asset (creates/modifies nodes atomically). "
				"Opens its own session when session_id is not provided.");
}

TSharedPtr<FJsonObject> ClaireonBehaviorTreeTool_ApplySpec::GetInputSchema() const
{
	FToolSchemaBuilder Builder;
	Builder.AddString(TEXT("asset_path"), TEXT("Path to the Behavior Tree asset to apply the spec to."), true);
	Builder.AddObject(TEXT("spec"), TEXT("Declarative spec object describing nodes[] with id/type/parent/children/decorators/services/properties."), true);
	Builder.AddString(TEXT("session_id"), TEXT("Optional existing session to reuse instead of opening a new one."));
	return Builder.Build();
}

FToolResult ClaireonBehaviorTreeTool_ApplySpec::Execute(const TSharedPtr<FJsonObject>& Arguments)
{
	FString AssetPath;
