...
FString FClaireonMetaSoundTool_AddOutput::GetOperation() const { return TEXT("add_output"); }
...
TSharedPtr<FJsonObject> FClaireonMetaSoundTool_AddOutput::GetInputSchema() const
{
	...
	S.AddString(TEXT("name"), TEXT("Output name"), true);
	S.AddString(TEXT("data_type"), TEXT("Output data type (Audio, Float, etc.)"), true);
	S.AddString(TEXT("default"), TEXT("Optional default literal value"));
	return S.Build();
}

IClaireonTool::FToolResult FClaireonMetaSoundTool_AddOutput::Execute(const TSharedPtr<FJsonObject>& Arguments)
{
#if !CLAIREON_HAS_METASOUND_BUILDER_API
	...
	{
		MSAddOutput_BuildLiteralFromJson(FName(*TypeStr), DefaultJson, Default);
	}

	FScopedTransaction Transaction(FText::FromString(TEXT("[Claireon] Add MetaSound Output")));
	Data->Asset->Modify();

	EMetaSoundBuilderResult Result = EMetaSoundBuilderResult::Failed;
	BuilderObj->AddGraphOutputNode(FName(*Name), FName(*TypeStr), Default, Result, /*bIsConstructorOutput=*/false);
	if (Result != EMetaSoundBuilderResult::Succeeded)
	{
		Transaction.Cancel();
		return MakeErrorResult(FString::Printf(TEXT("AddGraphOutputNode failed for '%s' (type %s)"), *Name, *TypeStr));
	}

	Data->bDirty = true;
	Data->LastOperationStatus = FString::Printf(TEXT("Added MetaSound output '%s' (%s)"), *Name, *TypeStr);

	TSharedPtr<FJsonObject> Out = MakeShared<FJsonObject>();
	Out->SetStringField(TEXT("session_id"), SessionId);
	Out->SetStringField(TEXT("output_name"), Name);
