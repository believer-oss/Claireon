...
{
    return TEXT("Disconnect a specific pin on a node (or all its connections) in the open Blueprint editing session. Requires open session_id from bp_open (or pass asset_path to auto-open). Transactional. The pin remains on the node; only the wires are removed. Accepts either session_id or asset_path; auto-opens a session when asset_path is supplied.");
}
