...
{
    return TEXT("Remove a dynamic pin from a node (Sequence/MakeArray/Switch/etc.) in the open Blueprint editing session. Requires open session_id from bp_open (or pass asset_path to auto-open). Transactional. Connections to the removed pin are dropped; non-dynamic pins error out unmodified. Accepts either session_id or asset_path; auto-opens a session when asset_path is supplied.");
}
